...
def main():
    ...
    # Đếm số parameters
    total_params = count_parameters(global_adapter, only_trainable=False)
    num_trainable = count_parameters(global_adapter, only_trainable=True)
    accelerator.print(
        f"GlobalAdapter params: {total_params} ({total_params / 1e6:.2f} M), "
    ...
    # Nếu muốn, có thể kiểm tra lại số trainable params sau khi prepare
    unwrapped_adapter = accelerator.unwrap_model(global_adapter)
    num_trainable_after = count_parameters(unwrapped_adapter, only_trainable=True)
    accelerator.print(
        f"Trainable params (after prepare): {num_trainable_after} "
        f"({num_trainable_after / 1e6:.2f} M)"
    )
