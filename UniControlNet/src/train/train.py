...
def main():
    ...
    # nếu muốn: auto chọn accelerator theo số GPU
    if gpus is None or gpus == 0:
        accelerator = None
    elif gpus == 1:
        accelerator = None       # 1 GPU thì không cần ddp
    else:
        accelerator = "ddp"      # >1 GPU -> dùng DDP
    ...
    dataloader = DataLoader(
        ...
        pin_memory=True,
        shuffle=False,  # hoặc bỏ hẳn arg này
    )
    ...
    # )
    trainer = pl.Trainer(
        gpus=gpus,                       # vd: --gpus 2 -> 2 GPU, --gpus -1 -> tất cả
        accelerator=accelerator,
        callbacks=[logger, checkpoint_callback],
        default_root_dir=default_logdir,
        max_steps=training_steps,

        # QUAN TRỌNG với IterableDataset (StreamingUniDataset)
