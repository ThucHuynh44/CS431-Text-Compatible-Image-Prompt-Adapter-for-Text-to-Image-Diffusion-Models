...
class UniControlNet(LatentDiffusion):
    ...
    def configure_optimizers(self):
        ...
        # Luôn đưa adapter vào optimizer
        if self.mode in ['local', 'uni']:
            params += list(self.local_adapter.parameters())
        if self.mode in ['global', 'uni']:
            params += list(self.global_adapter.parameters())

        # Nếu sd_locked=False thì train thêm một phần SD (như code gốc)
        if not getattr(self, "sd_locked", False):
            params += list(self.model.diffusion_model.output_blocks.parameters())
            params += list(self.model.diffusion_model.out.parameters())

        # Lọc theo requires_grad cho chắc
        params = [p for p in params if p.requires_grad]

        opt = torch.optim.AdamW(params, lr=lr)
        print("NUM OPT PARAMS:", sum(p.numel() for p in params))
        return opt
    ...
    # ==========================
    # Freeze / unfreeze SD gốc
    # ==========================
    def lock_sd(self):
        """Chỉ đảm bảo adapter có grad. KHÔNG đụng UNet/VAE/CLIP."""
        if self.mode in ['local', 'uni']:
            for p in self.local_adapter.parameters():
    ...
    def unlock_sd(self):
        """Nếu muốn cho phép train cả SD + adapter (mode cũ)."""
        # UNet
        ...
        if self.mode in ['global', 'uni']:
            for p in self.global_adapter.parameters():
                p.requires_grad = True
    # Đảm bảo khi bắt đầu train, trạng thái lock/unlock ăn theo self.sd_locked
